...
4. *Сервис оценивания*. Может аггрегировать информацию от сервиса запуска решений по отдельным пользователям, на основе этой информации по определенным формулам/алгоритмам рассчитывать оценки и прогресс по курсу и отдавать данные фронту и LMS.
...
4. *Сервис оценивания - фронтенд*. Скорее всего, оценки по курсу будут располагаться где-то на отдельной странице, поэтому достаточно синхронной интеграции. Используем REST, при открытии страницы с прогрессом по курсу отправляем GET-запросы и получаем нужную инфу.
